  ...
  <v-container class="cont">
    <v-row>
      <v-col class="box" cols="5">
        <h1>Task</h1>
        <v-btn class="show-btn" @click="showFormHandle(1)" >{{ showForm ? 'Close Add Form': 'Show Add Form' }}</v-btn>
        <TaskAddForm v-if="showForm" />
      </v-col>
      <v-spacer></v-spacer>
      <v-col class="box" cols="5">
        <h1>Application</h1>
        <v-btn class="show-btn" @click="showFormHandle(2)">{{ showForm2 ? 'Close Add Form': 'Show Add Form' }}</v-btn>
    ...
    <v-row style="display: flex; justify-content: center;">
      <v-col class="box" cols="5">
        <h1>Server</h1>
        <v-btn class="show-btn" @click="showFormHandle(3)">{{ showForm3 ? 'Close Add Form': 'Show Add Form' }}</v-btn>
